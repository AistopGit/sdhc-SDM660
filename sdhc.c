...

{
    ...
    *SlotCount = 0;
    ...
    //
    // This driver must not be run on devices connected via PCI.
    // 
    ...
    default:
        ...
        NT_ASSERT((BusType == SdBusTypeAcpi));
...

{
    ...
    //
    // PIO is currently broken, use DMA for all transfers.
    //
...

{
    ...
    case SdResetTypeAll:
        ...
        // 
        // QC-specific: Disable HC Select In on eMMC before doing a full reset.
        //

        VSFunc = SdhcReadRegisterUlong(SdhcExtension, SDHC_VENDOR_SPEC_FUNC);
        VSFunc &= ~SDHC_VS_FUNC_HC_SELECT_IN_MASK;
        SdhcWriteRegisterUlong(SdhcExtension, SDHC_VENDOR_SPEC_FUNC, VSFunc);

#ifdef DEBUG
        TraceEvents(TRACE_LEVEL_INFORMATION, 
                    TRACE_DEVICE, 
                    "SdhcResetHost: SdResetTypeAll");
#endif
        Mask = SDHC_RESET_ALL;
        break;
    ...
    default:
        ...
        TraceEvents(TRACE_LEVEL_ERROR, 
                    TRACE_DEVICE, 
                    "SdhcResetHost: Invalid reset type specified! Value: %d", 
                    ResetType);
#endif
	...
	//
	// QC-specific: save the Power Control register value to check if the 
    // SD bus was powered on before a full reset.
	//
	
    if (ResetType == SdResetTypeAll) {
        PwrCtl = SdhcReadRegisterUchar(SdhcExtension, SDHC_POWER_CONTROL);
    }
    ...
    //
    // QC-specific: if the SD bus was powered on before a full reset, handle power interrupts.
    //

    if ((PwrCtl & SDHC_PC_BUS_POWER) && ResetType == SdResetTypeAll) {
        SdhcQcHandlePowerInterrupts(SdhcExtension);
    }

    //
    // Poll the reset state.
    //
    ...
    do {
        ...
        if (Retries == 0) {
            ...
            TraceEvents(TRACE_LEVEL_ERROR,
                        TRACE_DEVICE, 
                        "SdhcResetHost: Timeout when resetting the host controller!");
#endif
...

{
    ...
    do {
        ...
        if (Retries == 0) {
            ...
            TraceEvents(TRACE_LEVEL_ERROR, 
                        TRACE_DEVICE, 
                        "SdhcSetClock: Timeout when checking the clock stable state!");
#endif
            return STATUS_IO_TIMEOUT;
        }
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, 
                TRACE_DEVICE, 
                "SdhcSetClock: New clock: %d, actual: %d", 
                Frequency, 
                ActualFrequency);
#endif
...

{
    ...
    //
    // This eMMC implementation only supports 1.8V. 
    // SDPort still wants to set 3.3V for some reason, even though
    // the Capabilities register doesn't report 3.0V/3.3V support.
    //
    ...
    */ 
    ...
    default:
        ...
        TraceEvents(TRACE_LEVEL_ERROR, 
                    TRACE_DEVICE, 
                    "SdhcSetVoltage: Invalid voltage specified! Value: %d", 
                    Voltage);
#endif
    ...
    do {
        ...
        if (Retries == 0) {
            ...
            TraceEvents(TRACE_LEVEL_ERROR, 
                        TRACE_DEVICE, 
                        "SdhcSetVoltage: Timeout when setting voltage!");
#endif
            return STATUS_IO_TIMEOUT;
        }
    ...
    do {
        ...
        if (Retries == 0) {
            ...
            TraceEvents(TRACE_LEVEL_ERROR, 
                        TRACE_DEVICE, 
                        "SdhcSetVoltage: Timeout when turning the bus power on!");
#endif
            return STATUS_IO_TIMEOUT;
        }
    ...
    //
    // QC-specific: handle power interrupts after setting the voltage.
    //
...

{
    ...
    default:
        ...
        TraceEvents(TRACE_LEVEL_ERROR,
                    TRACE_DEVICE, 
                    "SdhcSetBusWidth: Invalid bus width specified! Value: %d", 
                    Width);
#endif
...

{
    ...
    //
    // QC-specific: handle power interrupts after changing the signaling voltage.
    //
...

{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, 
                TRACE_DEVICE, 
                "SdhcGetInterruptStatus: Interrupt status: 0x%X", 
                InterruptStatus);
#endif
...

{
    ...
    ErrorStatus = SdhcReadRegisterUshort(SdhcExtension, SDHC_ERROR_STATUS);
#ifdef DEBUG
    TraceEvents(TRACE_LEVEL_WARNING, 
                TRACE_DEVICE, 
                "SdhcGetErrorStatus: Error status: 0x%X", 
                ErrorStatus);
#endif
...

{
    ...
    ErrorStatus = SdhcReadRegisterUshort(SdhcExtension, SDHC_AUTO_CMD12_ERROR_STATUS);
#ifdef DEBUG
    TraceEvents(TRACE_LEVEL_WARNING, 
                TRACE_DEVICE,
                "SdhcGetAutoCmd12ErrorStatus: Error status: 0x%X", 
                ErrorStatus);
#endif
...

{
    ...
    ErrorStatus = SdhcReadRegisterUshort(SdhcExtension, SDHC_ADMA_ERROR_STATUS);
#ifdef DEBUG
    TraceEvents(TRACE_LEVEL_WARNING, 
                TRACE_DEVICE, 
                "SdhcGetAdmaErrorStatus: Error status: 0x%X", 
                ErrorStatus);
#endif
...

{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, 
                TRACE_DEVICE, 
                "SdhcAcknowledgeInterrupts: Interrupts acknowledged.");
#endif
}
...
{
    //
    // This controller implementation has broken card detection 
    // (the Card Inserted bit in the Present State register is always 0).
    //
...

{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, 
                TRACE_DEVICE, 
                "SdhcSendCommand: CMD%d, Argument = 0x%X, ResponseType = 0x%X, TransferType = 0x%X", 
                Command->Index,
                Command->Argument,
...

{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, 
                TRACE_DEVICE, 
                "SdhcBuildTransfer: Transfer mode = 0x%X, block count = 0x%X, block size = 0x%X", 
                TransferMode, 
                BlockCount, 
                BlockSize);
#endif
...

{
    ...
    case SdTransferMethodPio:
        ...
        TraceEvents(TRACE_LEVEL_INFORMATION, 
                    TRACE_DEVICE, 
                    "SdhcStartTransfer: SdTransferMethodPio");
#endif
    ...
    case SdTransferMethodSgDma:
        ...
        TraceEvents(TRACE_LEVEL_INFORMATION, 
                    TRACE_DEVICE, 
                    "SdhcStartTransfer: SdTransferMethodSgDma");
#endif
        ...
        while (RemainingLength > 0) {
            ...
            *TotalTransferLength += NextLength;
            ...
            if (Use64BitDescriptor) {
                ...
                //
                // ARM64 requires strict alignment, but here the buffer pointer 
                // ends up being aligned only to 4 bytes, not 8. 
                // This causes a misalignment exception. To workaround this, we'd 
                // write the two 32-bit parts of the address one by one.
                // 
                
                //*((PULONGLONG) Buffer) = NextAddress.QuadPart;
                *((PULONG)Buffer) = NextAddress.LowPart;
                Buffer += sizeof(ULONG);
                *((PLONG)Buffer) = NextAddress.HighPart;
                Buffer += sizeof(LONG);
...
Routine Description:
    ...
    Handle vendor-specific power interrupts.
...

{
    ...
    do {
        if (Retries == 0) {
            // There are no power interrupts. Exit.
            return;
        }
        VSRegVal = SdhcReadRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_PWRCTL_STATUS);
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, 
                TRACE_DEVICE, 
                "SdhcQcHandlePowerInterrupts: Power control status value = 0x%X", 
                VSRegVal);
#endif
    ...
    // Clear the interrupt.
    //
    SdhcWriteRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_PWRCTL_CLEAR, VSRegVal);

    //
    // We're not saving the register value in this loop
    // as it's used later.
    //
    Retries = 100;
    do {
        if (Retries == 0) {
            ...
            TraceEvents(TRACE_LEVEL_ERROR, 
                        TRACE_DEVICE, 
                        "SdhcQcHandlePowerInterrupts: The vendor-specific power control interrupt did not clear!");
#endif
            break;
        }
        ...
        SdPortWait(1000);
        Retries--;
    } while (SdhcReadRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_PWRCTL_STATUS) != 0);

    //
    // Acknowledge all power interrupts.
    //
    if (VSRegVal & (SDHC_VS_PWRCTL_BUS_OFF_MASK | SDHC_VS_PWRCTL_BUS_ON_MASK)) {
        SdhcWriteRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_PWRCTL_CTL, SDHC_VS_PWRCTL_BUS_ONOFF_S_MASK);
    }

    if (VSRegVal & SDHC_VS_PWRCTL_IO_LOW_V_MASK) {
        VSRegVal = SdhcReadRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_FUNC);
        VSRegVal |= SDHC_VS_FUNC_IO_PAD_PWRSW_MASK;
        SdhcWriteRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_FUNC, VSRegVal);

        SdhcWriteRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_PWRCTL_CTL, SDHC_VS_PWRCTL_IO_SIGSW_S_MASK);
    } else if (VSRegVal & SDHC_VS_PWRCTL_IO_HIGH_V_MASK) {
        VSRegVal = SdhcReadRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_FUNC);
        VSRegVal &= ~SDHC_VS_FUNC_IO_PAD_PWRSW_MASK;
        SdhcWriteRegisterUchar(SdhcExtension, SDHC_VENDOR_SPEC_FUNC, VSRegVal);
